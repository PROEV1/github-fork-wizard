...
-- Create a storage bucket for client documents if not exists
...
ON storage.objects 
FOR ALL 
USING (bucket_id = 'client-documents' AND get_user_role(auth.uid()) = 'admin'::user_role);

CREATE POLICY "Admins can upload documents" 
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'client-documents' AND get_user_role(auth.uid()) = 'admin'::user_role);