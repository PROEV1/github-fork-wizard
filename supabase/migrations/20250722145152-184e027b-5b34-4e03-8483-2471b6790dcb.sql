-- Update the RLS policy to allow clients to see general conversation messages
...
USING (
  ...
  (quote_id IS NULL AND project_id IS NULL AND (
    ...
    get_user_role((SELECT sender_id FROM messages m2 WHERE m2.id = messages.id)) = 'admin'::user_role
  ))
);