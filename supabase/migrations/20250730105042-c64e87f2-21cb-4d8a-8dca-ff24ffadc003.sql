-- Update quote number generation function to start from 45100
...
BEGIN
    IF NEW.quote_number IS NULL OR NEW.quote_number = '' THEN
        -- Get the highest existing Q number or start from 45100
...
-- Update order number generation function to start from 35100
...
BEGIN
    IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
        -- Get the highest existing PS number or start from 35100
        SELECT COALESCE(
            MAX(
                CASE 
